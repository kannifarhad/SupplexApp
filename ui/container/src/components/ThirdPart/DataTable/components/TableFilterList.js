...
const TableFilterList = ({
  ...
  const filterComponents = serverSide && serverSideFilterList ? getFilterList(serverSideFilterList) : getFilterList(filterList);
  const isActiveFilter = filterComponents.reduce(function(sum, current) {
    return sum ? sum : current.length > 0
  }, false);
  return (
    isActiveFilter &&  
