...
import { assignWith as assignwith ,  cloneDeep, find, isEqual, isUndefined, merge} from 'lodash';
...
class MUIDataTable extends React.Component {
      ...
      for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
        ...
        if (typeof tableData[rowIndex] === 'undefined') {
          tableData.push({
            ...
            allData: status === TABLE_LOAD.INITIAL ? rawData[rowIndex] : rawData[rowIndex],
          });
        }
    ...
    for (let index = 0; index < row.length; index++) {
      ...
      if (filterVal.length || filterType === 'custom') {
        ...
        if (column.filterOptions && column.filterOptions.logic) {
          if (column.filterOptions.logic(columnValue, filterVal, tableMeta)) isFiltered = true;
  ...
  getTableMeta = (rowIndex, colIndex, rowData, columnData, tableData, curState, currentTableData) => {
    ...
    return {
      ...
      allData: (typeof currentTableData[rowIndex].allData !== 'undefined') ? currentTableData[rowIndex].allData :{}
    };
  };
