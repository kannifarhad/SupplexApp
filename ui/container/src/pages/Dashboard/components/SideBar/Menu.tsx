...
const MenuItem: React.FC<MenuItemProps> = ({ item, currentUrl, userRole }) => {
  ...
  const isActive = useMemo(() => isMenuItemActive(item), [isMenuItemActive, item]);
  ...
  if ((userRole && accessRole.length > 0 && !accessRole.includes(userRole)) || !item.showInMenu) {
    return null;
  }
  return (
    <ListItem
    ...
    >
      {hasChildren ? (
        <>
          <ListItem button className="asideMenuToggle" onClick={handleMouseClick}>
