...
import { createTokens, InputError, JwtPayload, removeRefreshCookie, UserError } from "../../utils";
import {
  AuthPayload,
  CreateUserInput,
  LoginInput,
...
export class UserResolver {
  ...
  user(@Ctx() ctx: Context, @Arg("where") where: UserWhereUniqueInput) {
  ...
  async refreshToken(@Ctx() ctx: Context) {
    ...
    const decodedRefresh = verify(refreshToken, constants.JWT_REFRESH_SECRET as string) as JwtPayload;
  ...
  @Mutation((_) => User, {
    description: "Updates users existing password",
  })
  async updateUserPassword(@Arg("input") input: UpdateUserPasswordInput, @Ctx() ctx: Context) {
    const { prisma } = ctx;
    if (!ctx.user) throw new UserError("UNAUTHENTICATED");
    ...
    const isPasswordMatch = await bcrypt.compare(input.previousPassword, targetUser.password);

    if (!isPasswordMatch) {
      throw new UserError("BAD_USER_INPUT", "Previous password is wrong");
    }
  ...
  async updateUser(@Arg("input") input: UpdateUserInput, @Arg("where") where: UserWhereUniqueInput, @Ctx() ctx: Context) {
    const { prisma } = ctx;
  ...
  async createUser(@Arg("input") input: CreateUserInput, @Ctx() ctx: Context) {
    ...
    if (targetUser) {
      throw new UserError("BAD_USER_INPUT", "User with this e-mail already exists in SUPPLEX!");
    }
